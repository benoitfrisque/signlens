...
def train(name_model=None):
    ...
    name_model = input(Fore.RED+"Enter the name of the model you want to load (if you want to reset the model press enter):"+Style.RESET_ALL)
    ...
    save_results(params=params, metrics=dict(val_accuracy=val_accuracy,val_loss=val_loss),params_path=paths["params_path"],metrics_path=paths["metrics_path"])
    save_model(model=model,model_path=paths["model_path"])
    return paths

def evaluate(name_model=None):

    name_model = input(Fore.RED+"Enter the name of the model you want to load:"+Style.RESET_ALL)
    ...
    save_results(params=params, metrics=dict(val_accuracy=accuracy,val_loss=loss),params_path=os.path.join(path,"params"),metrics_path=os.path.join(path,"metrics"),mode="evaluate")
    print("✅ evaluate() done \n")
